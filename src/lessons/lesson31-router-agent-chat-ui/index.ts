...
import { runRouterAgentStream } from "./orchestrator.js";
...
server.get("/health", async () => {
  return {
    status: "ok",
    lesson: "lesson31-router-agent-chat-ui",
...
server.post<{
  ...
  try {
    ...
    for await (const chunk of runRouterAgentStream({ message, sessionId })) {
      if (closed) {
        break;
      }
...
console.log(`Lesson 31 Router Agent Chat UI is running at http://localhost:${port}`);